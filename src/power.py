...
CWD = os.getcwd()
output_path = os.path.join(ROOT, "data_unif", "raw_data", "power")
...
if __name__ == "__main__":
    ...
    anneal_time = [4]  # crashed for 500 at 89 samples
    NUM_READS = 20
    initial_value = 1.0
    ...
    for chain_length in SCALING:
        filepath = os.path.join(ROOT, "data_unif", f"instance_{chain_length}.pkl")
        if os.path.exists(filepath):
            ...
            h = {node: 1 for node in range(chain_length)}
            J = {
                (node, node + 1): 1 for node in range(chain_length - 1)
            }
